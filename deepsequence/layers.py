...
from keras.layers import Layer, TimeDistributed, BatchNormalization, Activation, concatenate, Conv1D, GlobalMaxPool1D
...
def char_cnn_encode(input, n_gram_filter_sizes, n_gram_filter_nums):
    """
    character level cnn feature extraction
    :param input: char embedding
    :param n_gram_filter_sizes: ngram kernel filter sizes
    :param n_gram_filter_nums: ngram kernel filter nums
    :return:
    """
    cnn_outputs = []
    for ngram_size, filter_num in zip(n_gram_filter_sizes, n_gram_filter_nums):
        cnn_conv = TimeDistributed(Conv1D(filters=filter_num, kernel_size=ngram_size,
                                           padding='same', strides=1, name='cnn_conv_k{}'.format(ngram_size)))(input)
        cnn_bnorm = TimeDistributed(BatchNormalization(), name='cnn_bnorm_k{}'.format(ngram_size))(cnn_conv)
        cnn_act = TimeDistributed(Activation('tanh'), name='cnn_act_k{}'.format(ngram_size))(cnn_bnorm)
        cnn_pool = TimeDistributed(GlobalMaxPool1D(), name='cnn_pool_k{}'.format(ngram_size))(cnn_act)
        cnn_outputs.append(cnn_pool)

    char_cnn_out = concatenate(cnn_outputs, axis=-1, name='cnn_output')
